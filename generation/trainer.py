...
class Trainer():
    ...
    def _init_models(self, loader):
        ...
        if not self.scale or (math.floor(self.scale / 4) != math.floor((self.scale - 1) / 4)):
            ...
            # If we have multiple GPUs, use DataParallel to spread the discriminator model over all GPUs
        ...
        if self.init_generator:
            ...
            # Initialize the amplitude of the noise at this scale
            loader.dataset.amps = {'s0': torch.tensor(1.).to(self.args.device)}
        else:
            # add amp
            # This is the case when we are transitioning from scale k to scale k+1. 
            # We need to compute the amplitude of the noise at scale k+1.

            # Get the data from the current iteration
            ...
            # Compute the amplitude of the noise at scale k+1
            ...
            # add scale
            # Add the scale factor to the generator
    ...
    def _init_eval(self, loader):
        ...
        #   self.args: the command line arguments
        #   self.scale: the current scale
        #
        # Returns:
        #   loader: the data loader with the amplitude of the noise at each scale
        ...
        # 4. Add scales to the generator model
        ...
        # 6. Move the model and the amplitude of the noise to the specified device
        self.g_model = self.g_model.to(self.args.device)
    ...
    def _init_optim(self):
        # Initialize the optimizers and schedulers for the generator and discriminator models.

        # Generator optimizer
        #   We use Adam optimization with the learning rate specified in the command line arguments.
        #   The parameters being optimized are the parameters of the current scale of the generator model.
        ...
        #   Same as the generator optimizer, but for the discriminator model.
        self.d_optimizer = torch.optim.Adam(self.d_model.parameters(), lr=self.args.lr, betas=self.args.dis_betas)

        # Learning rate schedulers
        #   We use the StepLR scheduler from the torch.optim package.
        #   The step size is the interval (measured in number of epochs) between each learning rate decay.
        #   The gamma is the multiplicative factor used to decrease the learning rate.
        ...
        # Reconstruction loss
        #   We use the mean squared error (MSE) loss as the reconstruction loss.
